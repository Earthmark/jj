...
impl OnDiskCommit {
    ...
    /// Replace the current on-disk commit with `new_commit`.
    fn replace(self, new_commit: Commit) -> Result<Self, RunError> {
        let tree_state = Mutex::new(TreeState::load(
            self.commit.store().clone(),
            self.working_copy_dir.clone(),
        ...
        Ok(OnDiskCommit {
            ...
            working_copy_dir: self.working_copy_dir,
            tree_state,
        })
    }
}
...
fn create_working_copies(
    ...
    tracing::debug!(?base_path, "creating working copies in path: ");
...
async fn run_inner<'a>(
    ...
    while let Some(res) = command_futures.join_next().await {
        ...
        if should_quit {
            tracing::info!(?should_quit, "receiver is no longer available");
            break;
        }
    }
    Ok(())
...
pub fn cmd_run(ui: &mut Ui, command: &CommandHelper, args: &RunArgs) -> Result<(), CommandError> {
    ...
    tx.finish(ui, "run: rewrote {count} commits with {shell_command}")?;
